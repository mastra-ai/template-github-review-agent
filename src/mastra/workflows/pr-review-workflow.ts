  ...
  execute: async ({ inputData }) => {
    ...
    for (const file of inputData.files) {
      if (SKIP_PATTERNS.some((p) => p.test(file.filename))) {
        ...
        file.additions === 0 &&
        file.deletions < MIN_DELETION_ONLY_LINES
      ) {
        skippedFiles.push(file.filename)
      } else {
...
function batchFiles(
  ...
  for (const f of files) {
    ...
    if (
      batch.length > 0 &&
      (chars + size > BATCH_CHAR_BUDGET || batch.length >= BATCH_FILE_LIMIT)
    ) {
      batches.push(batch)
      batch = []
      chars = 0
...
const reviewFiles = createStep({
  id: 'review-files',
  description:
    'Review files using the code review agent (batched for large PRs)',
  inputSchema: categorizedSchema,
  outputSchema: reviewedSchema,
  execute: async ({ inputData, mastra }) => {
    const { owner, repo, pullNumber, pr, reviewableFiles, skippedFiles } =
      inputData
    ...
    function buildPrompt(batch: FileEntry[], batchIndex: number): string {
      ...
      const sections = batch
        .map((f) => buildFileSection(f, includeContent))
        .join('\n---\n\n')
...
For EACH file, return an entry with the filename and an array of issues found (empty array if none). Be specific with line numbers from the diff.`
    ...
    if (isLargePR) {
      const results = await Promise.all(
        batches.map((batch, i) => reviewBatch(batch, i))
      )
    ...
    const issuesSummary = fileReviews
      ...
      .map((fr) => {
        const issues = fr.issues
          .map(
            (i) =>
              `  - [${i.severity}/${i.category}] ${i.line ? `${fr.filename}:${i.line}` : fr.filename}: ${i.message}`
          )
          .join('\n')
        return `**${fr.filename}:**\n${issues}`
      })
